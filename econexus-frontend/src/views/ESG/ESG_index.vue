      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-black/40 border border-white/5 p-6 rounded-none relative group hover:border-eco-primary/30 transition-colors">
          ...
          <div class="flex justify-between items-start mb-4">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-eco-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064" /></svg>
          </div>
          <div class="text-3xl font-bold font-mono transition-all duration-500 origin-left inline-block"
               :class="isUpdating ? 'text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,0.8)] scale-[1.02]' : 'text-white'">
            {{ formatNumber(esgData.assets.co2_reduction, 3) }}
          </div>
          <span class="text-sm font-normal transition-colors ml-1" :class="isUpdating ? 'text-emerald-400/70' : 'text-gray-500'">tCO₂e</span>
        ...
        <div class="bg-black/40 border border-white/5 p-6 rounded-none relative group hover:border-eco-primary/30 transition-colors">
          ...
          <div class="text-3xl font-bold font-mono transition-all duration-500 origin-left inline-block"
               :class="isUpdating ? 'text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,0.8)] scale-[1.02]' : 'text-white'">
            {{ formatNumber(esgData.assets.pollution_interception, 1) }}
          </div>
          <span class="text-sm font-normal transition-colors ml-1" :class="isUpdating ? 'text-emerald-400/70' : 'text-gray-500'">kg (N/P/化学)</span>
        ...
        <div class="bg-black/40 border border-white/5 p-6 rounded-none relative group hover:border-yellow-400/30 transition-colors">
          ...
          <div class="text-3xl font-bold font-mono transition-all duration-500 origin-left inline-block"
               :class="isUpdating ? 'text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.8)] scale-[1.02]' : 'text-white'">
            {{ formatNumber(esgData.assets.carbon_revenue, 0) }}
          </div>
          <span class="text-sm font-normal transition-colors ml-1" :class="isUpdating ? 'text-yellow-400/70' : 'text-gray-500'">CNY</span>
        ...
        <div class="lg:col-span-1 bg-black/40 border border-white/5 rounded-none p-6 flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-5 font-mono max-h-[480px]">
            <div v-for="log in esgData.logs" :key="log.id" class="relative pl-4 border-l"
                 :class="log.category === '靶向干预' ? 'border-yellow-400/30' : (log.category === '闭环控制' ? 'border-blue-500/30' : 'border-emerald-500/30')">
              <span class="absolute -left-1.5 top-1 w-3 h-3 bg-black border rounded-full"
                    :class="log.category === '靶向干预' ? 'border-yellow-400' : (log.category === '闭环控制' ? 'border-blue-400' : 'border-emerald-400')"></span>
...
const esgData = ref({
  assets: { co2_reduction: 0, pollution_interception: 0, financial_rating: "AAA", carbon_revenue: 0 },
  logs: [],
  charts: {
    trend: { categories: [], baseline: [], actual: [] },
    reduction: []
  }
});
...
const fetchEsgDashboardData = async () => {
  try {
    ...
    // 【核心动画逻辑】：比对新旧数据，如果变大了，激活 1 秒钟的荧光闪烁动效
    if (esgData.value.assets.pollution_interception > 0 &&
        data.assets.pollution_interception > esgData.value.assets.pollution_interception) {
        isUpdating.value = true;
        setTimeout(() => { isUpdating.value = false; }, 1000); // 1秒后取消闪烁，平滑变回白色
    }

    esgData.value = data;
    nextTick(() => { updateCharts(); });
  } catch (error) {
    console.error("读取动态资产失败", error);
  }
};
...
const exportToPDF = async () => {
  ...
  if (!element) return;
  const opt = {
    margin: 0.1, filename: `天衍_ESG生态合规审计报告_2026.pdf`, image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true, backgroundColor: '#0a0c0a' }, jsPDF: { unit: 'in', format: 'a4', orientation: 'landscape' }
  };
...
const initCharts = () => {
  ...
  if (reductionChartRef.value && !reductionChart) reductionChart = echarts.init(reductionChartRef.value);
};

const updateCharts = () => {
  if (!trendChart || !reductionChart) return;
  const chartData = esgData.value.charts;
  if (!chartData) return;
  ...
  trendChart.setOption({
    ...
    xAxis: { type: 'category', data: chartData.trend.categories, axisLine: { lineStyle: { color: '#333' } }, axisLabel: { color: '#666', fontFamily: 'monospace' } },
    yAxis: { type: 'value', splitLine: { lineStyle: { color: '#222', type: 'dashed' } }, axisLabel: { color: '#666', fontFamily: 'monospace' } },
    series: [
      { name: '粗放经验基准排放', type: 'line', data: chartData.trend.baseline, symbol: 'none', lineStyle: { color: '#555', width: 2, type: 'dashed' } },
      { name: '模型调度实际排放', type: 'line', data: chartData.trend.actual, symbol: 'circle', symbolSize: 6, itemStyle: { color: '#10b981' }, lineStyle: { color: '#10b981', width: 2 } }
    ]
  });

  const colorMap = ['#3b82f6', '#10b981', '#eab308', '#ef4444'];
  const formattedBarData = chartData.reduction.map((item, idx) => ({
    value: item.value,
    itemStyle: { color: colorMap[idx % colorMap.length] }
  }));
  ...
  reductionChart.setOption({
    ...
    yAxis: { type: 'category', data: chartData.reduction.map(item => item.name), axisLine: { lineStyle: { color: '#333' } }, axisLabel: { color: '#888', fontFamily: 'monospace' } },
    series: [{
      name: '模型干预削减量', type: 'bar', barWidth: '30%',
...
onMounted(async () => {
  ...
  // 保持高频轮询
  setInterval(fetchEsgDashboardData, 3000);
